...
5. 浏览器访问 http://127.0.0.1:11950/admin/（实际端口取决于 .env 中的 FRONTEND_PORT）。
6. 运行 ./stop.sh 停止服务。
...
- `./issue-bridge-access-token.sh` 默认复用名为 `WeChat Bridge` 的 ACTIVE 设备，并写入一条 `EXPIRES_AT_ = NULL` 的 token 审计记录。
- 管理台与网关入口固定为 /admin/。
