...
if ($TtlSeconds -notmatch '^[0-9]+$') {
    throw 'ttl-seconds must be a positive integer'
}
...
try {
    ...
    Write-Output ("RUNNER_BEARER_TOKEN={0}" -f $token)
    Write-Output ("RUNNER_BEARER_EXPIRES_AT={0}" -f $exp)
} finally {
