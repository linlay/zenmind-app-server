...
mkdir -p "$BUNDLE_STAGE/images" "$BUNDLE_STAGE/config" "$BUNDLE_STAGE/data" "$BACKEND_BUILD_DIR" "$FRONTEND_BUILD_DIR" "$IMAGE_WORK_DIR"
...
write_runtime_registry "$BUNDLE_STAGE/config/config-files.runtime.yml"
